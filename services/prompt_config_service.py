...
class PromptConfigService:
    ...
    def _load_prompts(self):
        ...
        try:
            if self.prompts_config_file.exists():
                ...
                # Проверяем и добавляем недостающие дефолтные промпты
                self._add_missing_default_prompts()
            else:
    ...
    def _add_missing_default_prompts(self):
        """Добавляет недостающие дефолтные промпты в существующий файл"""
        default_prompts = self._get_default_prompts_dict()
        updated = False
        
        for prompt_key, prompt_data in default_prompts.items():
            if prompt_key not in self.prompts:
                # Добавляем недостающий промпт
                self.prompts[prompt_key] = prompt_data
                logger.info(f"Добавлен недостающий промпт: {prompt_key}")
                updated = True
            else:
                # Проверяем, есть ли все необходимые поля
                existing_prompt = self.prompts[prompt_key]
                for field in ['name', 'description']:
                    if field not in existing_prompt:
                        existing_prompt[field] = prompt_data.get(field, '')
                        updated = True
                # Проверяем наличие system_prompt или user_prompt
                if 'system_prompt' in prompt_data and 'system_prompt' not in existing_prompt:
                    existing_prompt['system_prompt'] = prompt_data['system_prompt']
                    updated = True
                if 'user_prompt' in prompt_data and 'user_prompt' not in existing_prompt:
                    existing_prompt['user_prompt'] = prompt_data['user_prompt']
                    updated = True
        
        if updated:
            self._save_prompts()
            logger.info("Обновлен файл промптов с недостающими дефолтными промптами")
    
    def _get_default_prompts_dict(self) -> Dict[str, Dict[str, str]]:
        """Возвращает словарь с дефолтными промптами"""
        return {
            "generate_post": {
