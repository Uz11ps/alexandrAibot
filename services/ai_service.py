        ...
        try:
            ...
            if not retry_success and self.proxy_enabled:
                ...
                try:
                    ...
                    # Добавляем temperature только если модель его поддерживает
                    if self.supports_temperature:
                        final_retry_params["temperature"] = 0.7
                    
                    response = await asyncio.wait_for(
                        client_without_proxy.chat.completions.create(**final_retry_params),
                        timeout=timeout_seconds
                    )
...
Будь максимально детальным и конкретным. Опиши все важные элементы, которые видны на кадре. Это кадр из видео, поэтому важно зафиксировать все детали для создания полного описания процесса."""
        ...
        try:
            ...
            # Подготовка сообщений для Vision
            if model_name.startswith("gpt-5") or "o1" in model_name.lower():
                # Для новых моделей объединяем инструкции
                messages = [
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": f"ИНСТРУКЦИЯ: Проанализируй этот кадр из видео со строительного объекта Археон.\n\nЗАДАНИЕ: {video_frame_prompt}"},
                            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}}
                        ]
                    }
                ]
...
Твоя задача - создавать глубокие, информативные и интересные посты для социальных сетей.

КРИТИЧЕСКИ ВАЖНЫЕ ТРЕБОВАНИЯ:
**СТРОГОЕ ОГРАНИЧЕНИЕ ДЛИНЫ: Текст поста должен быть в пределах 1500-2000 символов (включая пробелы и эмодзи). Сделай сообщение развернутым и детальным.**
- Используй эмодзи для визуального оформления (но не перегружай).
- Посты должны быть СТРУКТУРИРОВАННЫМИ (заголовки, списки).
- Стиль: дружелюбный, экспертный, современный.
- Избегай общих фраз, давай больше конкретики."""
...
{sources_context}"""
        ...
        try:
            logger.info(f"Генерация развернутого поста на основе анализа {len(source_posts)} источников")
            ...
            logger.error(f"Ошибка при генерации развернутого поста: {e}")
            return self._get_fallback_source_post()
