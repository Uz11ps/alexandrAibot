...
КРИТИЧЕСКИ ВАЖНО:
- Остальные абзацы (1, 2, 4) должны остаться БЕЗ ИЗМЕНЕНИЙ - скопируй их точно как в оригинале
- Измени ТОЛЬКО {paragraph_num}-й абзац согласно правкам выше
- НЕ переписывай весь пост заново
- НЕ изменяй другие абзацы
- Сохрани структуру из 4 абзацев
- Верни весь пост целиком: все 4 абзаца, где изменен только {paragraph_num}-й"""
            else:
                prompt = f"""Вот исходный пост (структура из 4 абзацев):
{original_post}
...
- Если просят изменить что-то конкретное, измени только это, остальное оставь максимально близко к оригиналу"""
            ...
            response = await asyncio.wait_for(
                self.client.chat.completions.create(
                    ...
                    temperature=0.3,  # Низкая температура для минимальных изменений
                    max_tokens=2500  # Больше токенов для сохранения структуры
                ),
