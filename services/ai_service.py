...
def clean_ai_response(text: str) -> str:
    ...
    # Удаляем технические заголовки "Черновик поста для согласования"
    # Удаляем HTML-теги из заголовков
    cleaned_text = re.sub(r'<b>📝\s*Черновик поста для согласования[^<]*</b>', '', cleaned_text, flags=re.IGNORECASE)
    cleaned_text = re.sub(r'<b>Черновик поста для согласования[^<]*</b>', '', cleaned_text, flags=re.IGNORECASE)
...
Будь конкретным и профессиональным."""
    ...
    async def refine_post_now(self, original_post: str, edits: str) -> str:
        ...
        cleaned_post = re.sub(r'<b>📝\s*Черновик поста для согласования[^<]*</b>', '', cleaned_post, flags=re.IGNORECASE)
        cleaned_post = re.sub(r'<b>Черновик поста для согласования[^<]*</b>', '', cleaned_post, flags=re.IGNORECASE)
        
        # Затем удаляем обычные заголовки (с эмодзи и без)
        header_patterns = [
            r'📝\s*Черновик поста для согласования[^:]*:?\s*\n*',
            r'📝\s*Полный текст ниже ⬇️\s*\n*',
            r'Черновик поста для согласования[^:]*:?\s*\n*',
        ...
        # Проверяем, просят ли удалить заголовки "Черновик поста для согласования"
        header_delete_keywords = ['черновик поста для согласования', 'черновик поста', 'заголовок']
        is_header_delete_request = any(keyword in edits.lower() for keyword in header_delete_keywords) and any(kw in edits.lower() for kw in ['убери', 'удали', 'убрать', 'удалить'])
        
        if is_header_delete_request:
            # Программно удаляем все заголовки из текста
            logger.info("Запрос на удаление заголовков - удаляем программно")
            result = original_post
            
            # Удаляем HTML-теги из заголовков
            result = re.sub(r'<b>📝\s*Черновик поста для согласования[^<]*</b>', '', result, flags=re.IGNORECASE)
            result = re.sub(r'<b>Черновик поста для согласования[^<]*</b>', '', result, flags=re.IGNORECASE)
            ...
            logger.info(f"Заголовки удалены программно. Исходная длина: {len(original_post)}, новая длина: {len(result)}")
            return result
...
Отредактируй ТОЛЬКО этот абзац согласно правкам. Сохрани стиль и содержание, внеси только необходимые изменения. Верни ТОЛЬКО отредактированный абзац."""
                ...
                # Очищаем от заголовков после замены
                # Удаляем HTML-теги из заголовков
                result = re.sub(r'<b>📝\s*Черновик поста для согласования[^<]*</b>', '', result, flags=re.IGNORECASE)
                result = re.sub(r'<b>Черновик поста для согласования[^<]*</b>', '', result, flags=re.IGNORECASE)
...
Напиши ТОЛЬКО один абзац о частых ошибках на данном этапе строительства. Абзац должен описывать типичные ошибки и их последствия. Верни ТОЛЬКО текст абзаца, без дополнительных комментариев."""
                    ...
                    # Очищаем от заголовков после вставки
                    # Удаляем HTML-теги из заголовков
                    result = re.sub(r'<b>📝\s*Черновик поста для согласования[^<]*</b>', '', result, flags=re.IGNORECASE)
                    result = re.sub(r'<b>Черновик поста для согласования[^<]*</b>', '', result, flags=re.IGNORECASE)
