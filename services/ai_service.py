...
class AIService:
    ...
    async def classify_content(self, prompt: str, photos_description: Optional[str] = None) -> str:
        """Определяет тип контента (expert, layout, report, meme)"""
        if not prompt and not photos_description:
            return "report"
            
        sys_prompt = "Ты — классификатор контента для застройщика Архион. Определи категорию на основе текста и описания фото/видео.\n\nКАТЕГОРИИ:\n- layout: Если на фото планировка, чертеж дома или обсуждается эргономика помещений.\n- expert: Если обсуждается ипотека, законы, рынок, или дается совет эксперта.\n- meme: Если контент юмористический, смешной или саркастичный.\n- report: Если это просто отчет о проделанной работе на стройплощадке (залили фундамент, поставили стены).\n\nВ ответе напиши ТОЛЬКО одно слово-ключ категории."
        user_msg = f"ТЕКСТ: {prompt}\nОПИСАНИЕ МЕДИА: {photos_description}"
        ...
        try:
            ...
            if cat in ["layout", "expert", "meme", "report"]:
                return cat
            return "report"
        except Exception:
            return "report"
    ...
    async def generate_post_text(self, prompt: str, context: Optional[str] = None, photos_description: Optional[str] = None, prompt_key: str = "generate_post") -> str:
        # Авто-классификация для выбора лучшего промпта
        if prompt_key == "generate_post":
            category = await self.classify_content(prompt, photos_description)
            if category == "layout":
                prompt_key = "layout_description"
            elif category == "meme":
                prompt_key = "meme_generation"
            elif category == "expert":
                # Если много источников, лучше использовать generate_from_sources
                if context and len(context) > 500:
                    prompt_key = "generate_from_sources"
                else:
...
5. ГРАММАТИКА: Проверяй окончания и согласования (например, «вторую льготную на дом не дадут»).
6. ЗАПРЕТ ЖАРГОНА: "клиентоцентричность", "кейс", "лид", "SLA".
7. ТИПОГРАФИКА: Только обычные дефисы (-)."""
