...
Будь конкретным и профессиональным."""
    ...
    async def refine_post_now(self, original_post: str, edits: str) -> str:
        ...
        # Разбиваем исходный пост на абзацы и удаляем абзацы, которые являются только заголовками
        paragraphs = []
        for p in original_post.split('\n\n'):
            p = p.strip()
            if not p:
                continue
        ...
        if is_delete_request and paragraph_nums_to_delete:
            ...
            # Удаляем HTML-теги из заголовков
            result = re.sub(r'<b>\s*📝\s*Черновик поста для согласования[^<]*</b>\s*\n*', '', result, flags=re.IGNORECASE | re.MULTILINE)
            result = re.sub(r'<b>\s*Черновик поста для согласования[^<]*</b>\s*\n*', '', result, flags=re.IGNORECASE | re.MULTILINE)
